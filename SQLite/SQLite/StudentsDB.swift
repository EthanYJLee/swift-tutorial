...
class StudentsDB{
    ...
    init(){
        // "studentsData.sqlite"가 어디 있는지 찾아오는
        let fileURL = try! FileManager.default.url(for: .documentDirectory, in: .userDomainMask, appropriateFor: nil, create: false).appending(path: "StudentsData.sqlite") // <- filename
    ...
    func tempInsert(){
        ...
        sqlite3_bind_text(stmt, 1, "유비", -1, SQLITE_TRANSIENT) // -1: length (blob 시 이미지 사이즈 지정)
    ...
    func insertDB(name:String,dept:String,phone:String) -> Bool{
        ...
        sqlite3_bind_text(stmt, 1, name, -1, SQLITE_TRANSIENT) // -1: length (blob 시 이미지 사이즈 지정)
    ...
    func updateDB(id:Int,name:String,dept:String,phone:String) -> Bool{
        var stmt: OpaquePointer?
        let SQLITE_TRANSIENT = unsafeBitCast(-1, to: sqlite3_destructor_type.self)
        let queryString = "UPDATE students (sname,sdept,sphone) SET (?,?,?) WHERE sid = ?"
        
        sqlite3_prepare(db, queryString, -1, &stmt, nil)
        sqlite3_bind_text(stmt, 1, name, -1, SQLITE_TRANSIENT) // -1: length (blob 시 이미지 사이즈 지정)
        sqlite3_bind_text(stmt, 2, dept, -1, SQLITE_TRANSIENT)
        sqlite3_bind_text(stmt, 3, phone, -1, SQLITE_TRANSIENT)
        sqlite3_bind_text(stmt, 4, String(id), -1, SQLITE_TRANSIENT)
